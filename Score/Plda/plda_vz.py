...
class PLDA(object):
    ...
    def log_likelihood_ratio(self, transform_train_ivector, num_utts,
        ...
        for i in range(self.dim):
            mean[i] = num_utts * self.psi[i] / (num_utts * self.psi[i] + 1.0) * transform_train_ivector[
                i]  # 单元素的nΨ/(nΨ+I) u ̅^g
    ...
    def get_stats_from_class_mean(self):
        ...
        for i in range(self.stats.num_classes):
            ...
            if info.num_example:
                ...
                self.within_var_stats += weight * n * np.matmul(m_w,
                                                                m_w.T)  # n_k(〖(Φ_b^(-1)+n〖Φ_w〗^(-1))〗^(-1)) + (w_k-m_k ) (w_k-m_k )^T
                self.within_var_count += weight

    def estimate_from_stats(self):

        self.within_var = (
                                      1.0 / self.within_var_count) * self.within_var_stats  # 1/K ∑_k▒〖n_k (Φ ̂_k+(w_k-m_k)〖(w_k-m_k)〗^T)〗
