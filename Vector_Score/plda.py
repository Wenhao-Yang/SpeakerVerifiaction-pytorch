...
class PldaStats(object):
    ...
    def is_sorted(self):
        for i in range(self.num_classes - 1):
            if self.class_info_[i].num_example > self.class_info_[i + 1].num_example:
                return False
    ...
    def sort(self):
        ...
        for i in range(self.num_classes - 1):
            for j in range(i + 1, self.num_classes):
                if self.class_info_[i].num_example > self.class_info_[j].num_example:
...
class PldaEstimator(object):
    ...
    def ComputeObjfPart2(self):
        ...
        for i in range(self.class_info_.size()):
    ...
    def ComputeObjf(self):
        ...
        print("Within-class objf per sample is %.6f between-class is %.6f, total is %.6f" % (
        ans1 / example_weights, ans2 / example_weights, normalized_ans))
    ...
    def ResetPerIterStats(self):
        self.within_var_stats_ = Resize(self.Dim())
        self.within_var_count_ = 0.0
        self.between_var_stats_ = Resize(self.Dim())
    ...
    def GetStatsFromClassMeans(self):
        ...
        for i in range(self.stats_.class_info_.size()):
            ...
            self.between_var_stats_ += weight * w
            self.between_var_count_ += weight

            self.within_var_stats_ += weight * n * mixed_var
            self.within_var_stats_ += weight * n * m_w.transpose() * m_w
            self.within_var_count_ += weight

    # M-step
    def EstimateFromStats(self, config, plda):
    ...
    def Estimate(self, config, plda):
        for i in range(config.num_em_iters):
            print("Plda estimation iteration %d of %d;" % (i, config.num_em_iters))
    ...
    def GetOutput(self, plda):
        plda.mean = 1 / self.stats_.class_weight_ * self.stats_.sum_
        ...
        plda.ransform_ = U.transpose() * transform1
        plda.psi_ = s  # 更新psi

        print("Diagonal of between-class variance in normalized space is ", s)
    #
