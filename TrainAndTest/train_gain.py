...
from Define_Model.FilterLayer import Back_GradCAM
from Define_Model.LossFunction import CenterLoss, Wasserstein_Loss, MultiCenterLoss, CenterCosLoss, AttentionMining
...
parser.add_argument('--fix-length', action='store_false', default=True, help='need to make mfb file')
...
parser.add_argument('--shuffle', action='store_false', default=True, help='using Cosine similarity')
...
parser.add_argument('--downsample', default=0, type=int, help='The channels of convs layers)')
...
def train(train_loader, model, back_guide, ce, optimizer, epoch):
...
def test(model, epoch, writer, xvector_dir):
    ...
    verification_extract(extract_loader, model, this_xvector_dir, epoch, args.test_input)
...
def main():
    ...
    model_kwargs = {'input_dim': args.input_dim, 'feat_dim': args.feat_dim, 'kernel_size': kernel_size,
                    'filter_fix': args.filter_fix, 'input_len': args.chunk_size, 'gain_layer': args.gain_layer,
                    ...
                    'downsample': args.downsample, 'groups': args.groups,
    ...
    ce = [ce_criterion, xe_criterion, ae_criterion]
    ...
    valid_loader = torch.utils.data.DataLoader(valid_dir, batch_size=max(int(args.batch_size / 2), 32), shuffle=False,
                                               **kwargs)
    ...
    if args.cuda:
        if len(args.gpu_id) > 1:
            ...
            torch.distributed.init_process_group(backend="nccl", init_method='tcp://localhost:32546', rank=0,
                                                 world_size=1)
            model = model.cuda()
    ...
    for epoch in range(start, end):
        ...
        train(train_loader, model, back_guide, ce, optimizer, epoch)
