...
if [ $stage -le 21 ]; then
  ...
  for sname in dev dev_aug_com; do
    ...
    python TrainAndTest/train_egs.py \
      ...
      --lr 0.1 \
      --milestones 10,20,40,50 \
      --check-path Data/checkpoint/${model}${resnet_size}/${datasets}/${feat_type}_egs_rvec/${loss}/input${input_norm}_${block_type}_${encoder_type}_em${embedding_size}_alpha${alpha}_wde4_${sname}_var \
      --resume Data/checkpoint/${model}${resnet_size}/${datasets}/${feat_type}_egs_rvec/${loss}/input${input_norm}_${block_type}_${encoder_type}_em${embedding_size}_alpha${alpha}_wde4_${sname}_var/checkpoint_10.pth \
      --kernel-size 5,5 \
      --channels 16,32,64,128 \
      --input-dim 161 \
      --block-type ${block_type} \
