...
def train(train_loader, model, teacher_model, ce, optimizer, epoch, scheduler):
    ...
    for batch_idx, (data, label) in pbar:
        ...
        if (batch_idx + 1) % args.log_interval == 0:
            epoch_str = 'Train Epoch {}: [ {:3.0f}% ]'.format(epoch, 100. * batch_idx / len(train_loader))
            ...
            epoch_str += ' Loss Teach: {:>9.6f}'.format(teacher_loss.float())
            epoch_str += ' Avg: {:.4f} '.format(total_loss / (batch_idx + 1))
