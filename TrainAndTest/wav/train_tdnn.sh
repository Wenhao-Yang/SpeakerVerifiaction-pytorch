...
if [ $stage -le 10 ]; then
  ...
  for lamda_beta in 0.2;do
    ...
    #  python TrainAndTest/train_egs/train_egs.py --train-config=TrainAndTest/wav/ecapa/vox2_int_brain_${data_type}.yaml --seed=${seed}
      ...
      CUDA_VISIBLE_DEVICES=0,5 OMP_NUM_THREADS=8 torchrun --nproc_per_node=2 --master_port=4175 TrainAndTest/train_egs/train_dist.py --train-config=TrainAndTest/wav/ecapa/last_epoch/vox1_brain_aug53_radionoise.yaml --seed=${seed}
