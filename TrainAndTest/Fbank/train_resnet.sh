...
#    CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node=2 TrainAndTest/train_egs_dist_mixup.py --train-config=TrainAndTest/Fbank/ResNets/aidata_resnet_mixup.yaml --seed=${seed}
  ...
  sleep 5
  CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node=2 TrainAndTest/train_egs/train_egs_dist_clsaug.py --train-config=TrainAndTest/Fbank/ResNets/vox1_aug/vox1_clsaug${layer}.yaml --seed=${seed} --lamda-beta 1.0 
...
if [ $stage -le 103 ]; then
 ...
 for seed in 123456 123457 123458 ; do
    ...
    CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node=2 TrainAndTest/train_egs/train_egs_dist.py --train-config=TrainAndTest/Fbank/ResNets/cnc1_resnet_light.yaml --seed=${seed}
