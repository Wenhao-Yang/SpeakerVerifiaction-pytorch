...
if [ $stage -le 44 ]; then
 ...
 for stat_type in margin1 ; do
   ...
   CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node=2 TrainAndTest/train_egs_distributed.py --train-config=TrainAndTest/Fbank/ResNets/aidata_resnet_mixup.yaml
  done
  exit
fi
