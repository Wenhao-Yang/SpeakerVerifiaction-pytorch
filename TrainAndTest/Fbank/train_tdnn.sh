...
#   CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node=2 TrainAndTest/train_egs_dist.py
   CUDA_VISIBLE_DEVICES=0,3 python -m torch.distributed.launch --nproc_per_node=1 TrainAndTest/train_egs_dist.py --train-config=TrainAndTest/Fbank/TDNNs/vox2_tdnn.yaml --seed=${seed}
  done
  exit
fi
