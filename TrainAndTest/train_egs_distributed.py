...
sys.stdout = NewLogger(osp.join(config_args.check_path, 'log.%s.txt' % time.strftime("%Y.%m.%d", time.localtime())))
...
def main():
    ...
    if config_args.loss_type == 'soft':
        ...
        xe_criterion = ArcSoftmaxLoss(margin=config_args.margin, s=config_args.s, iteraion=iteration,
                                      all_iteraion=config_args.all_iteraion, smooth_ratio=config_args.smooth_ratio,
    ...
    if len(config_args.random_chunk) == 2 and config_args.random_chunk[0] <= config_args.random_chunk[1]:
        ...
        train_loader = torch.utils.data.DataLoader(train_dir, batch_size=config_args.batch_size,
                                                   collate_fn=PadCollate(dim=pad_dim,
                                                                         num_batch=int(
                                                                             np.ceil(
                                                                                 len(train_dir) / config_args.batch_size)),
        ...
        train_loader = torch.utils.data.DataLoader(train_dir, batch_size=config_args.batch_size,
                                                   ...
                                                   **kwargs)
        valid_loader = torch.utils.data.DataLoader(valid_dir, batch_size=int(config_args.batch_size / 2), shuffle=False,
                                                   **kwargs)
