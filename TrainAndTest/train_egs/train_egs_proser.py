...
from Define_Model.Loss.SoftmaxLoss import AngleSoftmaxLoss, AngleLinear, AdditiveMarginLinear, AMSoftmaxLoss, \
    ArcSoftmaxLoss, \
    GaussianLoss, MinArcSoftmaxLoss, MinArcSoftmaxLoss_v2, ProserLoss
...
def main():
    ...
    if args.loss_type == 'soft':
        ...
        xe_criterion = ProserLoss(margin=args.margin, s=args.s, smooth_ratio=args.smooth_ratio,
                                  beta_alpha=0.2, gamma=args.proser_gamma, beta=args.proser_ratio,
                                  class_weight=class_weight)
    ...
    try:
        for epoch in range(start, end):
            ...
            if early_stopping_scheduler.early_stop:
                print('Best Epoch is %df:' % (early_stopping_scheduler.best_epoch))
