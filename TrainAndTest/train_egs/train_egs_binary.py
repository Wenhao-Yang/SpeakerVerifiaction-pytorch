...
from Define_Model.Loss.LossFunction import CenterLoss, MMD_Loss, aDCFLoss
...
from TrainAndTest.common_func import create_optimizer, create_model, verification_test, verification_extract, \
    args_model, save_model_args, args_parse
...
args = args_parse('PyTorch Speaker Recognition: Domain Binary Classification')
...
def main():
    ...
    xvector_kwargs = args_model(args, train_dir)

    keys = list(xvector_kwargs.keys())
    keys.sort()
    model_options = ["\'%s\': \'%s\'" % (str(k), str(xvector_kwargs[k])) for k in keys]
    ...
    if args.loss_type == 'soft':
        ...
        ce_criterion = aDCFLoss(alpha=args.s, beta=args.smooth_ratio, gamma=(1 - args.smooth_ratio), omega=args.margin)
