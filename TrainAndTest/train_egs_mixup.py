...
from Define_Model.Loss.SoftmaxLoss import AngleSoftmaxLoss, AngleLinear, AdditiveMarginLinear, AMSoftmaxLoss, \
    ArcSoftmaxLoss, \
    GaussianLoss, MinArcSoftmaxLoss, MinArcSoftmaxLoss_v2, SubArcSoftmaxLoss, MixupLoss
...
def train(train_loader, model, ce, optimizer, epoch, scheduler):
    ...
    for batch_idx, (data, label) in pbar:
        ...
        for width_mult in FLAGS.width_mult_list:
            ...
            minibatch_correct = lamda_beta * float((predicted_one_labels.cpu() == label[:len(data)].cpu()).sum().item()) \
                (1 - lamda_beta) * float((predicted_one_labels.cpu() == label[-len(data):].cpu()).sum().item())
