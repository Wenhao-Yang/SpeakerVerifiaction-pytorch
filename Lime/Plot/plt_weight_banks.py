...
ax.plot(xmore, ymore, marker='x', markevery=64)

ff = interpolate.interp1d(x, tim_fr)
tim_fr_enw = ff(xmore)
tim_fr_enw /= tim_fr_enw.sum()

ax.plot(xmore, tim_fr_enw, marker='o', markevery=64)
...
ax.plot(xmore, tim_so_enw, marker='^', markevery=64)
ax.legend(['Mel', 'F-ratio', 'Gradient'], fontsize=18)
ax.set_ylabel('Weight', fontsize=18)
ax.set_xlabel('(a) Scale Comparison', fontsize=22)
# ax.set_xticks([])
ax = fig.add_subplot(gs[2, 0])
for m in mel:
    ax.plot(xnew, m, color='#1f77b4', linewidth=0.8)
ax.set_ylabel('Weight', fontsize=18)
ax.set_xlabel('(b) Mel filters', fontsize=22)
# ax.set_xticks([])
...
# ax.set_title('')
ax.set_ylabel('Weight', fontsize=18)
ax.set_xlabel('(c) Gradient filters', fontsize=22)
pdf.savefig()
pdf.close()
plt.show()
