    ...
    def forward(self, X):
        if self.training:
            if self.alpha != 0.:
                ...
                return X * self.mask.repeat(tied_mask_shape).uniform_(1 - 2 * self.alpha, 1 + 2 * self.alpha).repeat(
                    repeats)
...
class FTDNN(nn.Module):
    ...
    def __init__(self, num_classes, input_dim=80, embedding_size=512, **kwargs):
        ...
        self.layer01 = TDNNLayer(input_dim=input_dim, output_dim=512, context_size=5, padding=2)
        ...
        self.layer12 = DenseReLU(4096, embedding_size)
        self.layer13 = DenseReLU(embedding_size, embedding_size)
    ...
    def forward(self, x):
        ...
        embeddings_b = self.layer13(embeddings_a)
